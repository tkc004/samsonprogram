...
def readsnap(sdir,snum,ptype,
    ...
    if (cosmological==1): print 'it is cosmological'
    ...
    if (ptype==0):
	if flag_cooling > 0:
		return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'u':ugas,'rho':rho,'h':hsml,'ne':nume,'nh':numh,'sfr':sfr,'z':metal,'cregy':cregy, 'header':newheader};
	else:
